function [IMAGE_PATHS_01, IMAGE_PATHS_02] = make_diffusion_images(dx_rand_list, num_pairs, image_size, img_repo)
...
% Save the displacement parameters to the jobfile.
...
% Save the "light" and "noise" parameters to the jobfile.
...
% Save the particle parameters to the jobfile
...
% Save the image sizes to the jobfile
...
% Allocate paths arrays
IMAGE_PATHS_01 = cell(num_pairs, num_diffusion_cases);
IMAGE_PATHS_02 = cell(num_pairs, num_diffusion_cases);
...
for n = 1 : num_diffusion_cases
    ...
    for k = 1 : num_pairs
        ...
        % Save the paths to the output variables
        IMAGE_PATHS_01{k, n} = image_path_01;
        IMAGE_PATHS_02{k, n} = image_path_02;
    ...
    end % End parfor(k = 1 : num_pairs);
    
    % Determine the name of the jobfile
    job_file_name = sprintf('%s.mat', case_name_current);
    
    % Determine the path on which to save the jobfile
    job_file_path = fullfile(job_file_dir, job_file_name);
    
    % Save the jobfile.
    save(job_file_path, 'JobFile');

end % End (for n = 1 : num_diffusion_cases)
