...
% standard deviations of the FT of the 
% displacements
FT_PDF_STD_DEV_Y = zeros(num_regions, 1);
FT_PDF_STD_DEV_X = zeros(num_regions, 1);
...
% Loop over all the interrogation regions.
    for k = 1 : num_regions
        ...
        for g = 1 : num_shuffles
            ...
            region_01 = extract_sub_regions(image_01,...
                [region_height, region_width], shifted_grid_x, shifted_grid_y);
            region_02 = extract_sub_regions(image_02,...
                [region_height, region_width], shifted_grid_x, shifted_grid_y);
...
parfor k = 1 : num_regions
    ...
    % mean displacement.
    [~, FT_PDF_STD_DEV_Y(k), FT_PDF_STD_DEV_X(k)] =...
        fit_gaussian_2D(abs(cc_div));
    
    % Convert the PDF standard deviations in to pixels per frame.
	DX_STD_DEV_Y(k) = (region_height)/ (2 * pi * FT_PDF_STD_DEV_Y(k));
	DX_STD_DEV_X(k) = (region_width) / (2 * pi * FT_PDF_STD_DEV_X(k));
